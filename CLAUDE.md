...
├── features/       # фичи — замкнутые модули (chat, ...)
│   └── chat/
...
- `index.ts` — только реэкспорт. Файлы с логикой называем по смыслу: `chatApi.ts`.
...
- Zod-схемы остаются в `api/*.ts` — они нужны для валидации ответа бека на системной границе. Тип выводится через `z.infer` и экспортируется, схема — нет.
